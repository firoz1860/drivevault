        ...
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          <thead className='text-gray-500 bg-white'>
            <tr>
              ...
              <th className="p-3 font-medium max-md:hidden">Checks</th>
          ...
          <tbody className='bg-white'>
            {bookings.length === 0 && (
              <tr>
                <td className='p-4 text-gray-500' colSpan={6}>No bookings available.</td>
              </tr>
            )}
            {bookings.map((booking, index)=>(
              <tr key={index} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3 max-md:hidden'>
                  <span className={`px-3 py-1 rounded-full text-xs ${booking.paymentStatus === 'paid' ? 'bg-green-100 text-green-600' : booking.paymentStatus === 'authorized' ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-500'}`}>
                ...
                <td className='p-3 max-md:hidden'>
                  <div className='flex flex-col gap-1 text-xs'>
                    <span className={`font-medium ${booking.checkInStatus === 'ready' ? 'text-green-600' : 'text-amber-600'}`}>{booking.checkInStatus || 'not-started'}</span>
                    <span className={`font-medium ${booking.digitalKey?.enabled ? 'text-amber-700' : 'text-gray-400'}`}>{booking.digitalKey?.enabled ? 'Digital key enabled' : 'Key locked'}</span>
                  </div>
                </td>
